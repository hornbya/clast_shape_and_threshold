...
//Silhouettes are measured by default. To change to image with holes, reactivate lines 48-52
	...
	parent=getDirectory("Select directory");
	...
	for (i = 0; i < list.length; i++) {
		...
		updateResults();
		savepath=parent+File.nameWithoutExtension+"-results.csv";
		savepath2=parent+File.nameWithoutExtension+"-labeled.tif";
		...
		waitForUser("Check labeled image");
		close("Results");
		roiManager("delete");
		close("*");'
	}